...
function App() {
  const { socketState, ibkrState, ibkrError, tickers, bars, tickerErrors } = useBackendSocket();
  ...
  return (
    <div className="mx-auto flex min-h-svh max-w-4xl flex-col gap-6 px-4 py-8">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Live 1-minute bar pipeline from IBKR TWS/Gateway — raw feed, no charting yet.
          </p>
        </div>
      ...
      <section className="flex flex-col gap-3">
        <h2 className="text-sm font-medium text-slate-300">Incoming bars ({bars.length})</h2>
        <BarTable bars={bars} />
      </section>
    </div>
  );
